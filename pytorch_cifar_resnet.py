...
class myNet(nn.Module):
    def __init__(self):
        ...
        self.channelup_conv1 = nn.Conv2d(3, 16, 3, 1, 1)
        self.conv3 = nn.Conv2d(16, 16, 3, 1, 1)
        self.conv4 = nn.Conv2d(16, 16, 3, 1, 1)
        self.channelup_conv2 = nn.Conv2d(16, 32, 3, 1, 1)
        self.conv5 = nn.Conv2d(32, 32, 3, 1, 1)
        self.conv6 = nn.Conv2d(32, 32, 3, 1, 1)
        self.channelup_conv3 = nn.Conv2d(32, 64, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.bn2(self.conv2(x)) + skip_connect
        x = F.relu(x)
        x = F.relu(self.channelup_conv1(x))
        x = F.max_pool2d(x, 2, 2)

        skip_connect = x
        x = F.relu(self.bn3(self.conv3(x)))
        x = self.bn4(self.conv4(x)) + skip_connect
        x = F.relu(x)
        x = F.relu(self.channelup_conv2(x))
        x = F.max_pool2d(x, 2, 2)

        skip_connect = x
        x = F.relu(self.bn5(self.conv5(x)))
        x = self.bn6(self.conv6(x)) + skip_connect
        x = F.relu(x)
        x = F.relu(self.channelup_conv3(x))
        x = F.max_pool2d(x, 2, 2)
...
best_model = myNet()
best_model.load_state_dict(torch.load('./cifar_best_model.pth'))
##########################################################################
